...
// startContainer runs a mongoDB container to execute commands.
func startContainer(t *testing.T) *container {
	t.Helper()

	cmd := exec.Command("docker", "run", "-P", "-d", "-e", "MONGO_INITDB_ROOT_USERNAME=root", "-e", "MONGO_INITDB_ROOT_PASSWORD=root", "mongo")
